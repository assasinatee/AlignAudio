...
class AudioGenerationDataset(AudioWaveformDataset, TaskMixin):
    ...
    def __post_init__(self, ):
        ...
        self.id_to_noisy_content = read_jsonl_to_mapping(
            self.noisy_content, id_col_in_content, self.content_col
        )
    ...
    def load_content_waveform(
        ...
        content_or_path_noisy = self.id_to_noisy_content[audio_id]

        if self.base_content_path:
            content_or_path = str(self.base_content_path / content_or_path)
            content_or_path_noisy = str(self.base_content_path / content_or_path)

        content, item_name = self.load_content(audio_id, content_or_path)
        noisy_content, noisy_item_name = self.load_content(audio_id, content_or_path_noisy)
        ...
        return content, waveform, duration, item_name, noisy_content
    ...
    def __getitem__(self, index) -> dict[str, Any]:
        ...
        content, waveform, duration, item_name, noisy_content = self.load_content_waveform(
            audio_id
        )
