...
from accel_hydra.models.common import LoadPretrainedBase, CountParamsBase, SaveTrainableParamsBase
...
def adaptive_l2_loss_with_mask(error, mask, gamma=0.5, c=1e-3):
    """
    Adaptive L2 loss: sg(w) * ||Δ||_2^2, where w = 1 / (||Δ||^2 + c)^p, p = 1 - γ
    Args:
        error: Tensor of shape (B, T, D)
        mask: Mask of audio latent
        gamma: Power used in original ||Δ||^{2γ} loss
        c: Small constant for stability
    Returns:
        Scalar loss
    """
    masked_error = error * mask[(..., ) + (None, ) * (error.ndim - mask.ndim)]
    delta_sq = masked_error.pow(2).sum(dim=tuple(range(1, error.ndim))) / mask.sum(dim=tuple(range(1, mask.ndim)))
    ...
    def get_input_target_and_timesteps(
        ...
        if training:
            if self.sample_strategy == 'normal':
                ...
                samples = torch.randn(bsz, 2)
                u = np.maximum(samples[:, 0], samples[:, 1])
                v = np.minimum(samples[:, 0], samples[:, 1])
            else:
        ...
        indices_u = (u * self.train_noise_scheduler.config.num_train_timesteps
                  ).long()
        indices_v = (v * self.train_noise_scheduler.config.num_train_timesteps
                  ).long()

        # train_noise_scheduler.timesteps: a list from 1 ~ num_trainsteps with 1 as interval
        timesteps = self.train_noise_scheduler.timesteps[indices_u].to(
...
class SingleTaskCrossAttentionAudioFlowMatching(
    ...
    def forward(
        self, content: list[Any], noisy_content: list[Any], condition: list[Any], task: list[str],
        ...
        noisy_content_dict = self.encode_content(noisy_content, task, device)
        noisy_content, noisy_content_mask = noisy_content_dict["content"], noisy_content_dict[
            "content_mask"]
        ...
        noisy_pred: torch.Tensor = self.backbone(
            x=noisy_latent,
            timesteps=timesteps,
            context=noisy_content,
            x_mask=latent_mask,
            context_mask=noisy_content_mask
        )

        # print(pred.shape, noisy_pred.shape)
        diff_loss = F.mse_loss(pred.float(), target.float(), reduction="none")

        t_norm = timesteps.float() / 1000

        # loss type: "l2" | "cosine"
        ALIGN_LOSS_TYPE = "cosine"
        # weight type: "none" | "early" | "middle" | "late"
        ALIGN_WEIGHT_TYPE = "early"

        k = 20
        if ALIGN_WEIGHT_TYPE == "none":
            align_weight = torch.ones_like(t_norm)
        elif ALIGN_WEIGHT_TYPE == "early":
            # high weight at early timesteps
            align_weight = torch.sigmoid((0.3 - t_norm) * k)
        elif ALIGN_WEIGHT_TYPE == "middle":
            # band-pass: emphasize middle timesteps
            align_weight = (
                torch.sigmoid((t_norm - 0.2) * k) *
                torch.sigmoid((0.6 - t_norm) * k)
            )
        elif ALIGN_WEIGHT_TYPE == "late":
            # high weight at late timesteps
            align_weight = torch.sigmoid((t_norm - 0.6) * k)
        
        if ALIGN_LOSS_TYPE == "l2":
            align_loss = F.mse_loss(
                noisy_pred.float(),
                pred.detach().float(),
                reduction="none"
            )
            align_loss = align_loss * align_weight.view(-1, 1, 1)
            align_loss = align_loss / (align_loss.detach().mean() + 1e-8)
        elif ALIGN_LOSS_TYPE == "cosine":
            v_clean = pred.detach()
            v_noisy = noisy_pred
            v_clean = F.normalize(v_clean, dim=-1)
            v_noisy = F.normalize(v_noisy, dim=-1)

            align_loss = 1.0 - (v_clean * v_noisy)
            align_loss = align_loss * align_weight.unsqueeze(1)
        
        diff_loss = loss_with_mask(diff_loss, latent_mask.unsqueeze(1), reduce=loss_reduce)
        align_loss = loss_with_mask(align_loss, latent_mask.unsqueeze(1), reduce=loss_reduce)

        output = {"diff_loss": diff_loss, "align_loss": align_loss}
        return output
